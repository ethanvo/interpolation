...
def calculate_emp2(ki, kj, ka, nkpts, kconserv, mo_coeff, mo_energy, nocc, nvir, ao2mo):
    kb = kconserv[ki, ka, kj]
    ci = mo_coeff[ki][:, :nocc]
    cj = mo_coeff[kj][:, :nocc]
    ca = mo_coeff[ka][:, nocc:]
    cb = mo_coeff[kb][:, nocc:]
    eia = mo_energy[ki][:nocc][:, None] - mo_energy[ka]
    ejb = mo_energy[kj][:nocc][:, None] - mo_energy[kb]
    ...
    emp2_local = 2 * lib.einsum('iajb,iajb', t2, iajb).real - lib.einsum('iajb,ibja', t2, ibja).real
    return emp2_local
...
if __name__ == '__main__':
    ...
    print("Begin parallel calculation")
    ...
    for ki, kj, ka in loop_kkk(nkpts):
        results.append(pool.apply_async(calculate_emp2, (ki, kj, ka, nkpts, kconserv, mo_coeff, mo_energy, nocc, nvir, mydf.ao2mo)))

    pool.close()
    pool.join()

    emp2 = sum(result.get() for result in results) / nkpts
