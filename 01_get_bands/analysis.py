...
size = berkelplot.fig_size(n_row=2, n_col=1)
fix, ax = plt.subplots(1, 1, figsize=size)
ax.plot(Nk, interp_data[0, 1:]-standard_data[1:], label="SCF 111", linestyle='--', marker="s")
ax.plot(Nk, interp_data[1, 1:]-standard_data[1:], label="SCF 222", linestyle=":", marker="^")
ax.plot(Nk[1:], interp_data[2, 2:]-standard_data[2:], label="SCF 333", linestyle="-.", marker="v")
ax.plot(Nk[2:], interp_data[3, 3:]-standard_data[3:], label="SCF 444", linestyle="-", marker="x")
ax.plot(Nk[3:], interp_data[4, 4:]-standard_data[4:], label="SCF 555", linestyle="-", marker="+")
ax.set_xlabel(r"$1/N_k$")
...
size = berkelplot.fig_size(n_row=2, n_col=1)
fix, ax = plt.subplots(1, 1, figsize=size)
ax.plot(Nk, interp_data[4, 1:]-standard_data[1:], label="SCF 555", linestyle="-", marker="+")
ax.plot(Nk[1:], interp_data[3, 2:]-standard_data[2:], label="SCF 444", linestyle="-", marker="x")
ax.plot(Nk[2:], interp_data[2, 3:]-standard_data[3:], label="SCF 333", linestyle="-.", marker="v")
ax.plot(Nk[3:], interp_data[1, 4:]-standard_data[4:], label="SCF 222", linestyle=":", marker="^")
ax.set_xlabel(r"$1/N_k$")
ax.set_ylabel("Interpolation Error (Ha)")
ax.set_xlim(0, 0.13)
ax.legend()
